...
export class DefectListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.markerService.markerID){
      ...
      this.subscriptionDefectFollow = this.defectService.getDefectsByFollowPlaces(parseInt(localStorage.getItem('loggedUserId'))).subscribe((data:Defect[]) => {this.defects = data; console.log(data)});
    } else{
